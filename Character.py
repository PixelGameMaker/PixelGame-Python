...
def character_surf_initialize():
    ...
    if str(choose_chara) == "Archer":
        player_images = {
            "standing": "assets/image/player/Archer/standf.png",
        ...
        player_images = {
            "standing": "assets/image/player/Knight/standf.png",
        ...
        player_images = {
            "standing": "assets/image/player/Magician/standf.png",
    ...
    bullet_images = {
        "normal_bullet": "assets/image/bullet/normal_bullet.png",
    ...
    size = (int(SCREEN_WIDTH / 33), int(SCREEN_WIDTH / 22))
    ...
    size = (int(displayInfo.current_h / 12), int(displayInfo.current_h / 12))
    ...
    size = (
            int(displayInfo.current_h * 0.005 * 2.5),
            int(displayInfo.current_h * 0.005 * 2.5),
        )
    bullet_surf['magic_particle'] = pygame.transform.smoothscale(bullet_surf['magic_particle'], size)
...
def get_update_direction(direction):
    ...
    elif abs(direction[0]) == abs(direction[1]) and abs(direction[0]) != 0:
        direction[0] /= abs(direction[1])
        direction[1] /= abs(direction[1])
    return direction
...
class Player(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.size = (int(displayInfo.current_h / 10), int(displayInfo.current_h / 10))
...
class Enemy(pygame.sprite.Sprite):
    ...
    def update(self, enemy_rect, enemy_direction, enemy_speed):
        ...
        speed = self.speed
        if (self.pos[0] - enemy_rect.x) ** 2 + (
                self.pos[1] - enemy_rect.y
        ) ** 2 <= self.stay_range ** 2:
            speed = -self.speed * 0.5

        self.pos[0] += (
                direction[0] * speed - enemy_direction[0] * enemy_speed - self.knockback[0]
        )
        self.pos[1] += (
                direction[1] * speed - enemy_direction[1] * enemy_speed - self.knockback[1]
        )
